(function() {
  ...
  _.each = function(collection, iterator) {
    ...
    if(Array.isArray(collection)){
      for(var i = 0; i < collection.length; i++){
        // apply the iterator on each item
  ...
  _.uniq = function(array, isSorted = true, iterator) {
    var results = [];
    var transformations = [];
    if (!iterator) {
      iterator = _.identity;
    ...
    for(var i = 0; i < array.length; i++){
      if(_.indexOf(transformations, iterator(array[i])) < 0){
        results.push(array[i]);
        transformations.push(iterator(array[i]));
      }
    }
    return results;
  };
  ...
  _.extend = function(obj) {
    //covert arugemnts into regular array
    //iterate through the arguments
    //add the properties to objects.
    //return object
    var args = [...arguments];
